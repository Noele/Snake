...
class Player:
    def __init__(self, settings: Settings):
        ...
        self.speed: float = settings.player_speed  # How much time in seconds, it takes for the player to move 1 space
    ...
    def draw(self, map: list[list[str]]) -> None:
        ...
        x, y = self.position
        map[y][x] = f"{self.head_colour}■{Colours.RESET}"
        for segment in self.tail_position:
            map[segment[1]][segment[0]] = f"{self.tail_colour}■{Colours.RESET}"
    ...
    def move(self, move_to: tuple[int, int]) -> None:
        if self.last_time_moved + self.speed < time.time() and self.alive:
            ...
            if move_to:
                dx = move_to[0] - self.position[0]
                dy = move_to[1] - self.position[1]
